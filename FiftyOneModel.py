...
train_df = pd.read_csv('FiftyOneTrain.csv')
train_df = train_df.sample(frac=1)

test_df = pd.read_csv('FiftyOneTest.csv')
...
def MLP():
	...
	#h = Dropout(dropout_percentage)(h)
	h = Reshape((1, 128))(h)
...
def Regressor():
	...
	#V = Dropout(dropout_percentage)(V)
...
def create_train_val(train_per, df):
	...
	for i in range(len(df)):
		if i < len(df) - val_len:
			y_train[i] = df.iloc[i, 153]
		else:
			y_val[val_len - len(df) + i] = df.iloc[i, 153]
...
#plt.savefig('FiftyOneModelCI.png')

#model.save('FiftyOneModel.h5')
